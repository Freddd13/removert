...
private:
    ...
    std::vector<std::string> sequence_valid_scan_paths2_;
    ...
    pcl::PointCloud<PointType>::Ptr map_global_full_dynamic_;  // the D_i
...
public:
    ...
    void transformGlobalMapToLocal(int _base_scan_idx, pcl::PointCloud<PointType>::Ptr cloud_to_process,
                                   bool what_the_fuck);
    void transformGlobalMapToLocalRevert(int _base_scan_idx);
...
std::vector<int> calcDiffRevertAndGetStaticIdx(const cv::Mat &_scan_rimg,
                                                                    const cv::Mat &_diff_rimg,
                                                                    const cv::Mat &_map_rimg_ptidx);

std::vector<int> calcDescrepancyRevert(std::pair<int, int> _rimg_shape,
                                                   pcl::PointCloud<PointType>::Ptr cloud_to_process);
}; // Removerter