...
void Removerter::cloudHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg) { cout << "TODO" << endl; }

void fsmkdir(std::string _path) {
  if (!fs::is_directory(_path) || !fs::exists(_path)) fs::create_directories(_path);  // create src folder
}  // fsmkdir
...
Removerter::Removerter() {
  ...
  // voxelgrid generates warnings frequently, so verbose off + ps. recommend to
  // use octree (see makeGlobalMap)
  pcl::console::setVerbosityLevel(pcl::console::L_ERROR);
  // pcl::console::setVerbosityLevel(pcl::console::L_ALWAYS);

  nh.param<int>("removert/num_nn_points_within", kNumKnnPointsToCompare,
                3);  // using higher, more strict static
...
void Removerter::parseValidScanInfo(void) {
  ...
  for (int curr_idx = 0; curr_idx < int(sequence_scan_paths_.size()); curr_idx++) {
    ...
    sequence_valid_scan_paths2_.emplace_back(sequence_scan_paths2_.at(curr_idx));
...
// for target range of scan idx
{
  ...
  assert(sequence_valid_scan_paths_.size() == sequence_valid_scan_paths2_.size());
  // for (auto &_scan_path : sequence_valid_scan_paths_) {
  for (int i = 0; i < sequence_valid_scan_paths_.size(); i++) {
    ...
    pcl::PointCloud<PointType>::Ptr points2(new pcl::PointCloud<PointType>);  // pcl::PointCloud Ptr is a shared ptr
                                                                             // so this points will be automatically
                                                                             // destroyed after this function block
                                                                             // (because no others ref it).

    // ROS_WARN("path1: %s", _scan_path1.c_str());
    // ROS_WARN("path2: %s", _scan_path2.c_str());
    ROS_BREAK();
    if (isScanFileKITTIFormat_) {
      // ROS_WARN("Reading bin...");
      readBin(_scan_path1, points1);  // For KITTI (.bin)
    } else {
      // ROS_WARN("Reading pcd...");
      pcl::io::loadPCDFile<PointType>(_scan_path1,
                                      *points1);  // saved from SC-LIO-SAM's pcd binary (.pcd)
    }

    if (isScanFileKITTIFormat_) {
      // ROS_WARN("Reading bin...");
      readBin(_scan_path2, points2);  // For KITTI (.bin)
    } else {
      // ROS_WARN("Reading pcd...");
      pcl::io::loadPCDFile<PointType>(_scan_path2,
                                      *points2);  // saved from SC-LIO-SAM's pcd binary (.pcd)
    }
...
#pragma omp parallel for num_threads(kNumOmpCores)
  for (int pt_idx = 0; pt_idx < num_points; ++pt_idx) {
    ...
    // @ Theoretically, this if-block would have race condition (i.e., this is a
    // critical section),
    ...
    // (2020.10.28)
    // @ Reason: because this for loop is splited by the omp, points in a single
    // splited for range do not race among them,
    // @         also, a point A and B lied in different for-segments do not
    // tend to correspond to the same pixel, #               so we can assume
    // practically there are few race conditions.
...
void Removerter::makeGlobalMap(void) {
  ...
  // - For a large-size point cloud should use OctreePointCloudVoxelCentroid
  // rather VoxelGrid
  ...
  *original_map = *cloud_tmp;
  ...
  if (kFlagSaveMapPointcloud) {
    ...
    // in local coord (i.e., base_node_idx == 0 means a start idx is the
    // identity pose)
...
std::pair<pcl::PointCloud<PointType>::Ptr, pcl::PointCloud<PointType>::Ptr> Removerter::removeDynamicPointsOfScanByKnn(
    int _scan_idx) {
  ...
  // again global2local because later in the merging global map function, which
  // requires scans within each local coord.
...
void Removerter::saveMapPointcloudByMergingCleanedScans(void) {
  ...
  // dynamic map
  {
    ...
    ROS_INFO_STREAM(
        "\033[1;32m  [For verification] A dynamic pointcloud "
        "(cleaned scans merged) is saved (global coord): "
        << local_file_name << "\033[0m");
